...
func TestExternalMCPListToolsUsesSessionID(t *testing.T) {
...
`))
	...
	client, err := newExternalMCPClient(context.Background(), mcpServer.URL)
	if err != nil {
		t.Fatalf("newExternalMCPClient() error = %v", err)
	}
...
func TestExternalMCPToolsExposeSafeNamesAndCallOriginalName(t *testing.T) {
...
`))
	...
	wrapped, ok := tools[0].(*externalMCPTool)
	if !ok {
		t.Fatalf("tool type = %T, want *externalMCPTool", tools[0])
	}
	info, err := wrapped.Info(context.Background())
	if err != nil {
		t.Fatalf("Info() error = %v", err)
