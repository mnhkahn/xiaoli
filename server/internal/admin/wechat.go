...
func (s *AdminServer) startWechatPolling(ctx context.Context) {
	...
	if token == "" {
		log.Printf("[wechat] WECHAT_BOT_TOKEN not set, skipping")
		return
	}
	...
	buf := ""
	log.Printf("[wechat] polling started base_url=%s", c.baseURL)

	for {
		select {
		case <-ctx.Done():
			log.Printf("[wechat] polling stopped")
			return
		default:
		}
		...
		if err := json.Unmarshal(raw, &resp); err != nil {
			log.Printf("[wechat] poll decode error: %v", err)
			continue
		}
		if resp.Ret != 0 {
			if resp.Ret == -14 {
				log.Printf("[wechat] session expired, need re-login")
				return
			}
			continue
		}
...
func (s *AdminServer) handleWechatMessage(ctx context.Context, c *wechatClient, msg *wechatMessage) {
	...
	log.Printf("[wechat] message from=%s text=%q", msg.FromUserID, text)

	if msg.ContextToken == "" || msg.FromUserID == "" || msg.ToUserID == "" {
		log.Printf("[wechat] message missing context_token/from/to, ignored")
		return
	}

	if s.conversation == nil {
		log.Printf("[wechat] conversation not configured")
		return
	}

	reply, err := s.conversation.Run(ctx, WechatTextFactory{}.Build(msg.ContextToken, msg.FromUserID, text))
	if err != nil {
		log.Printf("[wechat] conversation error: %v", err)
...
func wechatLogin(ctx context.Context, onQRCode func(content string)) error {
	...
	for attempt := 0; attempt < 3; attempt++ {
		...
		if onQRCode != nil && qrResp.QRCodeImgContent != "" {
			...
			log.Printf("[wechat] scan QR to login: %s", qrResp.QRCodeImgURL)
		}
	...
	loop:
		for {
			...
			case <-ctx.Done():
				return ctx.Err()
			case <-timeout:
				log.Printf("[wechat] QR expired, retrying (%d/3)", attempt+1)
				break loop
			case <-ticker.C:
				...
				case "confirmed":
					log.Printf("[wechat] login successful!")
					fmt.Printf("\nWECHAT_BOT_TOKEN=%s\nWECHAT_BASE_URL=%s\n", status.BotToken, status.BaseURL)
					return nil
				case "expired":
					break loop
				case "scaned":
					log.Printf("[wechat] QR scanned, waiting for confirmation")
				}
			}
		}
	}
