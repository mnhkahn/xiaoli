...
func (s *deviceSession) startVoiceRecording(mode string) {
	...
	if s.vad == nil {
		v, err := NewSileroVAD()
		if err != nil {
			logger.Infof("silero vad init failed for %s: %v", s.deviceID, err)
		} else {
			s.vad = v
		}
	}
...
func (s *deviceSession) appendVoiceFrame(payload []byte) (recv, voice, total int) {
	...
	if s.vad != nil {
		...
		if ran && s.audioRecvCnt%25 == 0 {
			logger.Infof("vad sample %s: prob=%.2f isVoice=%v voiceCnt=%d/%d", s.deviceID, prob, isVoice, s.audioVoiceCnt, s.audioRecvCnt)
		}
	} else {
